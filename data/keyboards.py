...
def start_inline_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "🎬 ဇာတ်လမ်း တစ်ပုဒ်ပဲ VIPဝင်မယ်", callback_data="main_buy_single"
                )
            ],
            [
                InlineKeyboardButton(
                    "📦 ဇာတ်လမ်း 15ပုဒ်  VIPဝင်မယ် - 5000 ကျပ်",
                    callback_data="main_buy_bundle",
                )
            ],
        ]
    )
...
def single_video_selection_keyboard(
    videos: List[Dict[str, Any]], page: int = 0, page_size: int = 5
) -> InlineKeyboardMarkup:
    ...
    if page > 0:
        nav_buttons.append(
            InlineKeyboardButton("⬅️ ရှေ့သို့", callback_data=f"page:{page - 1}")
        )
    if end_idx < len(videos):
        nav_buttons.append(
            InlineKeyboardButton("နောက်သို့ ➡️", callback_data=f"page:{page + 1}")
        )
...
def join_request_admin_keyboard(chat_id: int, user_id: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "✅ Approve Join", callback_data=f"jr:approve:{chat_id}:{user_id}"
                ),
                InlineKeyboardButton(
                    "❌ Reject Join", callback_data=f"jr:reject:{chat_id}:{user_id}"
                ),
            ]
        ]
    )
...
def broadcast_segment_keyboard(counts: Dict[str, int]) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    f"All users ({counts.get('all', 0)})", callback_data="bc:seg:all"
                )
            ],
            [
                InlineKeyboardButton(
                    f"Paid users ({counts.get('paid', 0)})", callback_data="bc:seg:paid"
                )
            ],
            [
                InlineKeyboardButton(
                    f"No-order users ({counts.get('no_order', 0)})",
                    callback_data="bc:seg:no_order",
                )
            ],
            [
                InlineKeyboardButton(
                    f"Single buyers ({counts.get('single', 0)})",
                    callback_data="bc:seg:single",
                )
            ],
            [
                InlineKeyboardButton(
                    f"Bundle buyers ({counts.get('bundle', 0)})",
                    callback_data="bc:seg:bundle",
                )
            ],
...
def after_payment_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "🔙 ပင်မစာမျက်နှာသို့ ပြန်သွားမည်", callback_data="back_to_main"
                )
            ]
        ]
    )
...
def delete_confirm_keyboard(video_id: str) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "✅ ဖျက်မည်", callback_data=f"del_confirm:{video_id}"
                ),
    ...
    for v in videos:
        ...
        buttons.append(
            [
                InlineKeyboardButton(
                    f"{has_link} {v['title']}",
                    callback_data=f"setlink_select:{v['id']}",
                )
            ]
        )
    ...
    for v in videos:
        ...
        buttons.append(
            [
                InlineKeyboardButton(
                    f"{has_id} {v['title']}",
                    callback_data=f"setchannelid_select:{v['id']}",
                )
            ]
        )
    buttons.append(
        [InlineKeyboardButton("❌ ပယ်ဖျက်မည်", callback_data="setchannelid_cancel")]
    )
    return InlineKeyboardMarkup(buttons)


def view_video_selection_keyboard(videos: List[Dict[str, Any]]) -> InlineKeyboardMarkup:
    """List of all videos for admin view selection."""
    buttons = []
    for v in videos:
        link_status = "✅" if v.get("channel_link") else "❌"
        id_status = "✅" if v.get("channel_id") else "❌"
        buttons.append(
            [
                InlineKeyboardButton(
                    f"{link_status}{id_status} {v['title']}",
                    callback_data=f"view_select:{v['id']}",
                )
            ]
        )
    buttons.append([InlineKeyboardButton("❌ ပယ်ဖျက်မည်", callback_data="view_cancel")])
    return InlineKeyboardMarkup(buttons)
